...
export function resolveTemplate(targetUrl) {
  ...
  for (const [, config] of templates) {
    ...
    if (domains.some((d) => url.hostname === d || url.hostname === `www.${d}`)) {
      matched = config;
      break;
    }
  }
...
export function scrapeMeta(root) {
  ...
  return {
    ...
    author:
      ...
      getMeta("name", "author") ||
      "",
    release: getMeta("property", "og:image:release"),
    installs: getMeta("property", "og:image:installs"),
    codeowners: getMeta("property", "og:image:codeowners"),
  };
}
